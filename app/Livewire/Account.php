...
class Account extends Component
{
    ...
    public function assignUserModalOpen(){
        $this->assignUserModal = true;
    }
    public function assignUserModalClose(){
    ...
    public function assignCompanies()
    {
        ...
        $this->selected = [];
        flash()->addSuccess('Companies assigned successfully!');
    }
    ...
    ->select('d.company_id', 'd.status', 'd.created_at')
    ...
    ->toSql();
        ...
        'u2.name as mname',
        DB::raw('latest_dispositions.status as disposition_status'),
        DB::raw('latest_dispositions.created_at as disposition_date'),
    ...
    ->groupBy('e.id', 'e.name', 'e.website', 'e.fax', 'users.name', 'assign_companies.user_id', 'u2.name', 'latest_dispositions.status','latest_dispositions.created_at');
           ...
           ->orderByDesc('e.created_at')
           ->groupBy('e.id', 'e.name', 'e.website', 'e.fax', 'users.name','e.created_at','u2.name','latest_dispositions.status','latest_dispositions.created_at');
            }elseif($this->unassign == "aa"){
                $dtt = Carbon::now()->subMonths($this->time_period)->toDateString();
                $accountssss = DB::table('assign_companies as ac')
            ->select('c.id', 'c.name', 'c.website', 'c.fax','u2.name as mname', 'users.name as user_name','c.created_at',
            ...
            ->leftJoin('company_locations', 'company_locations.company_id', '=', 'c.id')
            ->leftJoin('countries', 'countries.id','=','company_locations.country_id')
            ->leftJoin(DB::raw("({$latestDispositionSubquery}) as latest_dispositions"), 'latest_dispositions.company_id', '=', 'c.id')
            ->leftJoin('states', 'states.id','=','company_locations.state_id')
            ->leftJoin('clients', 'clients.companyId', '=', 'c.id')
            ->leftJoin('cities', 'cities.id', '=', 'company_locations.city_id')
           ->leftJoin('clients_phones', 'clients_phones.clients_id', '=', 'clients.id')
           ...
           ->groupBy('c.id', 'c.name', 'c.website', 'c.fax', 'users.name','c.created_at','u2.name','ac.assign_by','latest_dispositions.status','latest_dispositions.created_at');
            }else{
                $dtt = Carbon::now()->subMonths($this->time_period)->toDateString();
           ...
           ->leftJoin('clients_phones', 'clients_phones.clients_id', '=', 'clients.id')
            ...
            ->groupBy('c.id', 'c.name', 'c.website', 'c.fax', 'users.name','c.updated_at','c.created_at','u2.name','latest_dispositions.status','latest_dispositions.created_at');
            }
        }elseif($this->unassign == "na"){
            $accountssss = DB::table('companies as e')
            ->select('e.id', 'e.name', 'e.website', 'e.fax',
           ...
           ->orWhere('clients_phones.phone', 'like', '%' . $this->search .'%')
           ->orderByDesc('e.created_at')
           ->groupBy('e.id', 'e.name', 'e.website', 'e.fax', 'users.name','e.created_at','u2.name','latest_dispositions.status');
        }elseif($this->unassign == "aa"){
            $accountssss = DB::table('assign_companies as ac')
            ->select('c.id', 'c.name', 'c.website', 'c.fax','u2.name as mname', 'users.name as user_name','c.created_at',
            DB::raw('latest_dispositions.status as disposition_status'),
           ...
           ->where('c.name', 'like', '%' . $this->search . '%')
            ...
            ->orWhere('company_emails.email', 'like', '%' . $this->search . '%')
            ->orWhere('clients_phones.phone', 'like', '%' . $this->search .'%')
           ->groupBy('c.id', 'c.name', 'c.website', 'c.fax', 'users.name','c.created_at','u2.name','ac.assign_by', 'latest_dispositions.status');
        }elseif($this->unassign == "ua"){
            $accountssss = DB::table('companies as c')
            ->select('c.id', 'c.name', 'c.website', 'c.fax','c.created_at',DB::raw('latest_dispositions.status as disposition_status'),
            DB::raw('latest_dispositions.created_at as disposition_date'),
            DB::raw('GROUP_CONCAT(countries.shortname) as couname'),
            DB::raw('GROUP_CONCAT(states.name) as stname'),
            DB::raw('GROUP_CONCAT(cities.name) as cityname'),
           ...
           ->leftJoin(DB::raw("({$latestDispositionSubquery}) as latest_dispositions"), 'latest_dispositions.company_id', '=', 'c.id')
            ...
            ->groupBy('c.id', 'c.name', 'c.website', 'c.fax','c.created_at', 'latest_dispositions.status');
        }else{
            $accountssss = DB::table('dispositions as d')
            ->select('c.id', 'c.name', 'c.website','u2.name as mname', 'c.fax',
            'users.name as user_name','c.updated_at','c.created_at',
        ...
        $account = $accountssss->paginate(10);
        $users = User::select('id', 'name', 'last_seen')
        ->where('flag',0)
        ->where(function($query){
            $query->orWhere('name', 'like', '%' . $this->searchUser . '%');
        })->get();
