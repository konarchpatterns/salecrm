...
class UpdateClient extends Component
{
    ...
    public function mount(){
        ...
        if(count($phone)>0){
            ...
            {
                $this->inputs->push(['phone' =>$phone[$key]['phone'],
        ...
        if(count($email)>0){
            foreach($email as $key=>$val){
                $this->emailinputs->push(['email' =>$email[$key]['mail'],
        ...
        if(count($arrs)>0){
            foreach($this->inputs as $input){
                ...
                if($flag){
                    ...
                    if($input['phone'] ){
                        clients_phone::where('id',$tableId)->update([
                            'phone' => $input['phone'],
                            'type' => $input['phonetype']
                        ]);
                    }else{
                        clients_phone::where('id',$tableId)->delete();
                    }
            ...
            foreach($this->emailinputs as $mail){
                ...
                if($flag){
                    ...
                    if($mail['email'] && $mail['emailtype']){
                        clients_email::where('id',$tableId)->update([
                            'mail' => $mail['email'],
                            'type' => $mail['emailtype']
                        ]);
                    }else{
                        clients_email::where('id',$tableId)->delete();
                    }
