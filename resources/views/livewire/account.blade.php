    ...
    <section class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
        ...
        <div
            ...
            <div>
                ...
                <!-- Modal toggle -->
                <button wire:click="assignUserModalOpen()" type="button" class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-4 py-2 text-center me-2 mb-2">Assign</button>
                
@if ($assignUserModal)
  <div class="fixed inset-0 bg-gray-300 bg-opacity-60 overflow-y-auto z-40 flex items-center justify-center">
    <div class="relative p-4 w-full max-w-2xl max-h-full ">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <!-- Modal header -->
            <div class="sticky top-0 bg-gray-300 flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                    Assign User
                </h3>
                <button wire:click="assignUserModalClose()" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="static-modal">
            ...
            <div class="p-4 md:p-5 space-y-4">
                ...
                <div class="relative">
                    ...
                    <input type="search" id="default-search" wire:model.live.debounce.300ms="searchUser" class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search User" required />
                </div>

                <ul class="h-[40vh] py-2 overflow-y-auto text-gray-700 dark:text-gray-200" aria-labelledby="dropdownUsersButton">
                    @foreach ($users as $user)
                    @if(Cache::has('user-is-online-' . $user->id))
                    <li wire:key="{{$user->id}}">
                      <a href="#" data-modal-target="static-modal" data-modal-toggle="static-modal" wire:click="assignOpenModal('{{ $user->id }}')" class="relative flex items-center gap-2 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                        <div class="absolute ml-8 mt-5 h-2.5 w-2.5 rounded-full bg-[#4dcf35] me-2"></div>
                    ...
                    @if(!Cache::has('user-is-online-' . $user->id))
                    <li wire:key="{{$user->id}}">
                        <a href="#" data-modal-target="static-modal" data-modal-toggle="static-modal" wire:click="assignOpenModal('{{ $user->id }}')" class=" flex items-center gap-2 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                          <img class="h-10 w-10 rounded-full"
                                              src="https://avatar.iran.liara.run/public/boy?username={{ $user->name }}"
                                              alt="" />
                          <p>{{ $user->name }}</p>
                ...
                <tbody>
                    @foreach ($account as $accounts)
                        ...
                        <tr wire:key="{{ $accounts->id }}"
                            ...
                            <td class="px-6 py-4">
                                <a href="account/view/{{ $accounts->id }}">{{ $accounts->id }}</a>
                            </td>

                            <td class="px-6 py-4">
                                <a style="display: inline-flex;" href="account/view/{{ $accounts->id }}"
                            ...
                            <td class="px-6 py-4">
                                @php
                                    try {
                                        echo $accounts->disposition_status;
                                    } catch (\Throwable $th) {
                            ...
                            <td class="px-6 py-4">
                                @php
                                    try {
                                        echo $accounts->disposition_date;
                                    } catch (\Throwable $th) {
                                        echo $accounts->disposition_date;
                                    }
                                @endphp
